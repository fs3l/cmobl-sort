...
int32_t nob_read_dummy(struct CodaIntf_t* intf, uint32_t handle, uint32_t pos, int32_t* v) {
  *v = intf->nob_mem[intf->nob_start[handle]+pos];
  return 0;
}
int32_t nob_write_dummy(struct CodaIntf_t* intf, uint32_t handle, uint32_t pos,int32_t v) {
...
uint32_t addOb_dummy(struct CodaIntf_t* intf, int32_t* ob, uint32_t size) {
  for(int i=0;i<size;i++)
    intf->ob_mem[intf->ob_global+i] = ob[i];
  intf->ob_handle++;
...
uint32_t addNob_dummy(struct CodaIntf_t* intf, int32_t* nob, uint32_t size) {
  for(int i=0;i<size;i++)
    intf->nob_mem[intf->nob_global+i] = nob[i];
  intf->nob_handle++;
...
int32_t coda_finish_dummy(CodaIntf* intf) {
  return 0;
}
