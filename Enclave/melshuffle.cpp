...
__attribute__((always_inline)) void app_distribute_128d2_c(int32_t* txmem,
                                                           int size1, int size2)
//__attribute__((always_inline)) inline void app_distribute_c(int32_t* txmem,
//                                                            int size1,
//                                                            int size2)
{
}
...
{
  for (int i = 0; i < 2 * SqrtN * BLOWUPFACTOR; i++)
    bsortMem[i] = data[i / (2 * BLOWUPFACTOR) * 2 * BLOWUPFACTOR * SqrtN +
                       i % (2 * BLOWUPFACTOR) + data_init * 2 * BLOWUPFACTOR];
  *txmem_p = bsortMem;
  *size_p = 2 * SqrtN * BLOWUPFACTOR;
}
                           ...
                           int32_t** txmem_p, int32_t* size_p)
{
  ...
  for (int i = 0; i < 2 * BLOWUPFACTOR * SqrtN + 4 * SqrtN; i++) txmem[i] = -1;
                           ...
                           int32_t M_output_init, int32_t M_output_size)
{
  ...
  app_distribute_128d2_c(txmem1, 2 * BLOWUPFACTOR + SqrtN + SqrtN, M_data_size);
  app_distribute128d2();
  txend();
  for (int i = 0; i < SqrtN; i++)
                          ...
                          int32_t M_output_init, int32_t M_output_size)
{
  ...
  int32_t txmem_size;
  g_starts++;
  memsetup_distribute64d2(M_data, M_data_init, M_data_size, M_perm, M_perm_init,
                          M_output, M_output_init, M_output_size, &txmem1,
                          &txmem_size);
  contextsave();
                      ...
                      int32_t M_output_init, int32_t M_output_size)
{
  ...
  for (int i = 0; i < SqrtN; i++)
    for (int j = 0; j < 2 * BLOWUPFACTOR; j++) {
      M_output[M_output_init * SqrtN * 2 * BLOWUPFACTOR + i * 2 * BLOWUPFACTOR +
               j] = txmem[i * (2 * BLOWUPFACTOR + 1) + j + 1];
    }
}
